...
#if defined(NDEBUG)
# error "Poobcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcce66d4073d98a6831da740523d925a2753246fc7a43ea275e45033b805d520c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Poobcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Poobcoin Signed Message:\n";
...
{
    // Poobcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Poobcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Poobcoin: 1 days
static const int64 nTargetSpacing = 15 * 60; // Poobcoin: 15 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Poobcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Poobcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x460a47c443b05b554dac4d3b05da9f639956ff7d09ef7a31f1fb9fc86321c9cb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "poobcoin launches 4/26/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f1c20b5f2a0bf9594894c00e817efa6fcffa6cd78d42f9c1b82e11f4e8d4ea7f2f47fd050ac9eae4e3d9c042ab15163c7ddb8f97845026d49a22db7bff1f1c77") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524732899;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 386345750;

        if (fTestNet)
        {
            block.nTime    = 1524732800;
            block.nNonce   = 385461526;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x8f9013fb84d70113962a86df5fdfb1098e1d2e7dec9d660267de808c1c814e21"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xcf, 0xb4, 0xd0 }; // Poobcoin: random.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PoobcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PoobcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PoobcoinMiner(CWallet *pwallet)
{
    printf("PoobcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("poobcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PoobcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PoobcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PoobcoinMiner, pwallet));
}
